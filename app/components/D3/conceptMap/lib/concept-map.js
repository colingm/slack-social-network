var ConceptMap = function(chartElementId, infoElementId, dataJson){
	...
	var B=d.append("g").attr("class","links"),f=d.append("g").attr("class","users"),E=d.append("g").attr("class","nodes");
	...
	T.get("users").forEach(function(aa){
    ...
    i = Math.floor(c / T.get("users").length);
    y = Math.floor(T.get("users").length * i / 2);
    T.get("users").forEach(function(af, ae) {
      af.x = U / -2;
      af.y = ae * i - y
    });
    var ad = 180 + J,
      Z = 360 - J,
      ac = (Z - ad) / (T.get("leftNodes").length - 1);
    T.get("leftNodes").forEach(function(af, ae) {
      af.x = Z - ae * ac;
      af.y = h / 2 - R;
      af.xOffset = -S;
      af.depth = 1
    ...
    ad = J;
    Z = 180 - J;
    ac = (Z - ad) / (T.get("rightNodes").length - 1);
    T.get("rightNodes").forEach(function(af, ae) {
			af.x = ae * ac + ad;
			af.y = h / 2 - R;
      af.xOffset = S;
      af.depth = 1
    ...
    H = [];
    var ab, Y, aa, X = h / 2 - R;
    T.get("users").forEach(function(ae) {
    ...
    D(T.get("users"));
    b(d3.merge([T.get("leftNodes"), T.get("rightNodes")]));
    C([]);
    n();
    z()
	}
    ...
    if (ab && ab.type === "user") {
      ...
      if (ab && ab.type === "leftNode") {

      } else {
        if (ab && ab.type === "rightNode") {
    ...
    E.selectAll("circle").attr("fill", function(X) {
      ...
      if (X === L.node) {
        ...
        if (X.type === "leftNode" || X.type === "rightNode") {
          return 1.5
        }
      }
      return null
